...
void VkApp::submitFrame()
{
    ...
    auto result = vkQueueSubmit(m_queue, 1, &submitInfo, m_waitFence);
    if (result != VK_SUCCESS) 
    {
		std::cout << "vkQueueSubmit failed: " << result << std::endl; 
    }
    ...
    if (result != VK_SUCCESS)
    {
        std::cout << "vkQueuePresentKHR failed: " << result << std::endl;
    }
}
