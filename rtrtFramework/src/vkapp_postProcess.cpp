...
void VkApp::createPostFrameBuffers()
{
    ...
    for (uint32_t i = 0; i < m_imageCount; i++)
    {
        ...
        auto result = vkCreateFramebuffer(m_device, &createInfo, nullptr, &m_framebuffers[i]);
...
void VkApp::createPostPipeline()
{
    ...
    VkResult result = vkCreatePipelineLayout(m_device, &createInfo, nullptr, &m_postPipelineLayout);
    if (result != VK_SUCCESS)
    {
